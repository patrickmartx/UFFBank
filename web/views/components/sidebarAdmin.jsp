<div class="sidebar">
  <div>
    <div class="logo" href="/UFFBank/LoginAdmin">
      <img 
        src="contents/imgs/logo.png" 
        alt="logo da UFFBANK"
      />

      <h1>
        UFF<span>BANK</span>
      </h1>
    </div>

    <nav class="menu">
      <a width="24" height="22" class="link" href="/UFFBank/LoginAdmin">
        <svg 
        width="26" height="26"
        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="#676767"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path fill="none" d="M0 0h24v24H0z"></path> <path d="M14 14.252v2.09A6 6 0 0 0 6 22l-2-.001a8 8 0 0 1 10-7.748zM12 13c-3.315 0-6-2.685-6-6s2.685-6 6-6 6 2.685 6 6-2.685 6-6 6zm0-2c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm7 6.586l2.121-2.122 1.415 1.415L20.414 19l2.122 2.121-1.415 1.415L19 20.414l-2.121 2.122-1.415-1.415L17.586 19l-2.122-2.121 1.415-1.415L19 17.586z"></path> </g> </g></svg>
            
        Contas inativas
      </a>
      <a class="link" href="/UFFBank/ListActiveAccounts">
        <svg width="24" height="24" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clip-path="url(#clip0_48_365)">
          <path d="M4 28V25.3333C4 23.9188 4.5619 22.5623 5.5621 21.5621C6.56229 20.5619 7.91885 20 9.33333 20H14.6667C16.0812 20 17.4377 20.5619 18.4379 21.5621C19.4381 22.5623 20 23.9188 20 25.3333V28M21.3333 4.17333C22.4806 4.46707 23.4974 5.13427 24.2235 6.06975C24.9497 7.00523 25.3438 8.15577 25.3438 9.34C25.3438 10.5242 24.9497 11.6748 24.2235 12.6103C23.4974 13.5457 22.4806 14.2129 21.3333 14.5067M28 28V25.3333C27.9932 24.1562 27.5972 23.0144 26.8737 22.0859C26.1502 21.1573 25.1398 20.4943 24 20.2M17.3333 9.33333C17.3333 12.2789 14.9455 14.6667 12 14.6667C9.05448 14.6667 6.66667 12.2789 6.66667 9.33333C6.66667 6.38781 9.05448 4 12 4C14.9455 4 17.3333 6.38781 17.3333 9.33333Z" stroke="#2C3E50" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </g>
          <defs>
          <clipPath id="clip0_48_365">
          <rect width="32" height="32" fill="white"/>
          </clipPath>
          </defs>
          </svg>
          
          
        Contas ativas
      </a>

      
    </nav>
  </div>

  <form action="/UFFBank/LogoutAdmin" method="post" class="link">
    <button type="submit">
        <svg width="26" height="26" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_55_67)">
                <path d="M18.6667 10.6666V7.99992C18.6667 7.29267 18.3857 6.6144 17.8856 6.1143C17.3855 5.6142 16.7072 5.33325 16 5.33325H6.66667C5.95942 5.33325 5.28115 5.6142 4.78105 6.1143C4.28095 6.6144 4 7.29267 4 7.99992V23.9999C4 24.7072 4.28095 25.3854 4.78105 25.8855C5.28115 26.3856 5.95942 26.6666 6.66667 26.6666H16C16.7072 26.6666 17.3855 26.3856 17.8856 25.8855C18.3857 25.3854 18.6667 24.7072 18.6667 23.9999V21.3333" stroke="#2C3E50" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M9.33333 16H28M28 16L24 12M28 16L24 20" stroke="#2C3E50" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </g>
            <defs>
                <clipPath id="clip0_55_67">
                    <rect width="32" height="32" fill="white"/>
                </clipPath>
            </defs>
        </svg>
        Sair
    </button>
</form>

</div>